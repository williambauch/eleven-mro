...
function mGerarPagina1NR($pdf, $data)
{
    // =========================================================
        // Pagina 1 — Capa Não Rotina de JIC (Job Instruction Card)
        ...
        $pdf->Text(166.69, 154.25, $data['prev_nrc_first'] ?: ''); // new-1 - 01 List if applicable - NRC anterior (primeira)
        $pdf->Text(166.69, 161.66, $data['prev_nrc_last'] ?: ''); // new-2 - 02 List if applicable - NRC anterior (ultima)
        // Fonte: 11px bold -> 8.25pt
        $pdf->SetFont('helvetica', 'B', 8.25);
        ...
        $pdf->Text(99.48, 166.42, ''); // p077 - Revisão da Referência - (Revisão da Referência:)
        ...
        // Fonte: 11px normal -> 8.25pt
        $pdf->SetFont('helvetica', '', 8.25);
        $pdf->Text(82.02, 248.71, ''); // p121 - Assinatura do Cliente - Aqui Assinatura do Cliente
        $pdf->Text(82.02, 234.95, '(' . $var_customer . ')'); // p111 - - - (Nome Completo do Cliente)
        $pdf->Text(162.72, 249.5, ''); // p122 - Assinatura do IIO se necessário - Assinatura do IIO se necessário:
